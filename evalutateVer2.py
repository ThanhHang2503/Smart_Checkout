...
class ArcFaceLoss(nn.Module):
    ...
    def forward(self, embeddings, labels):
        ...
        theta = torch.acos(torch.clamp(logits, -1.0 + 1e-7, 1.0 - 1.0 - 1e-7))
...
def evaluate_model(model, pairs, threshold=0.5):
    ...
    predictions = []
    true_labels = []
    distances = []  # Để vẽ phân bố khoảng cách
    ...
    with torch.no_grad():
        for img1_path, img2_path, label in pairs:
            ...
            # Lưu điểm số cho ROC và phân bố khoảng cách
    ...
    precision = precision_score(true_labels[::2], predictions[::2])
    recall = recall_score(true_labels[::2], predictions[::2])
    f1 = f1_score(true_labels[::2], predictions[::2])
    ...
    cm = confusion_matrix(true_labels[::2], predictions[::2])
    
    return accuracy, precision, recall, f1, fpr, tpr, roc_auc, distances, cm
...
facenet_accuracy, facenet_precision, facenet_recall, facenet_f1, facenet_fpr, facenet_tpr, facenet_roc_auc, facenet_distances, facenet_cm = evaluate_model(facenet_model, test_pairs)
...
resnet18_accuracy, resnet18_precision, resnet18_recall, resnet18_f1, resnet18_fpr, resnet18_tpr, resnet18_roc_auc, resnet18_distances, resnet18_cm = evaluate_model(resnet18_model, test_pairs)

# 8. In kết quả
print(f"FaceNet - Accuracy: {facenet_accuracy:.4f}, Precision: {facenet_precision:.4f}, Recall: {facenet_recall:.4f}, F1-Score: {facenet_f1:.4f}, ROC-AUC: {facenet_roc_auc:.4f}")
print(f"ResNet18+ArcFace - Accuracy: {resnet18_accuracy:.4f}, Precision: {resnet18_precision:.4f}, Recall: {resnet18_recall:.4f}, F1-Score: {resnet18_f1:.4f}, ROC-AUC: {resnet18_roc_auc:.4f}")
...
# 10. Vẽ biểu đồ so sánh Accuracy, Precision, Recall, F1-Score
metrics_df = pd.DataFrame({
    'Model': ['FaceNet', 'FaceNet', 'FaceNet', 'FaceNet', 'ResNet18+ArcFace', 'ResNet18+ArcFace', 'ResNet18+ArcFace', 'ResNet18+ArcFace'],
    'Metric': ['Accuracy', 'Precision', 'Recall', 'F1-Score', 'Accuracy', 'Precision', 'Recall', 'F1-Score'],
    'Value': [facenet_accuracy, facenet_precision, facenet_recall, facenet_f1, resnet18_accuracy, resnet18_precision, resnet18_recall, resnet18_f1]
})

plt.figure(figsize=(10, 6))
sns.barplot(x='Value', y='Metric', hue='Model', data=metrics_df)
plt.title("Comparison of FaceNet and ResNet18+ArcFace Metrics")
plt.savefig("results/metrics_comparison.png")
plt.show()

# 11. Vẽ biểu đồ phân bố khoảng cách Embedding
facenet_same = [d for d, label in facenet_distances if label == 1]
facenet_diff = [d for d, label in facenet_distances if label == 0]
resnet18_same = [d for d, label in resnet18_distances if label == 1]
resnet18_diff = [d for d, label in resnet18_distances if label == 0]

plt.figure(figsize=(10, 6))
sns.kdeplot(facenet_same, label="FaceNet (Same Identity)", color="blue")
sns.kdeplot(facenet_diff, label="FaceNet (Different Identity)", color="red")
sns.kdeplot(resnet18_same, label="ResNet18+ArcFace (Same Identity)", color="green")
sns.kdeplot(resnet18_diff, label="ResNet18+ArcFace (Different Identity)", color="orange")
...
sns.heatmap(facenet_cm, annot=True, fmt="d", cmap="Blues", xticklabels=["Different", "Same"], yticklabels=["Different", "Same"])
plt.title("Confusion Matrix - FaceNet")
plt.xlabel("Predicted")
plt.ylabel("True")
plt.savefig("results/facenet_confusion_matrix.png")
plt.show()

plt.figure(figsize=(6, 4))
sns.heatmap(resnet18_cm, annot=True, fmt="d", cmap="Oranges", xticklabels=["Different", "Same"], yticklabels=["Different", "Same"])
plt.title("Confusion Matrix - ResNet18+ArcFace")
plt.xlabel("Predicted")
plt.ylabel("True")
plt.savefig("results/resnet18_confusion_matrix.png")
plt.show()